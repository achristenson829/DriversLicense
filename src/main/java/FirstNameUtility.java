...
public class FirstNameUtility {
    ...
    public int encodeFirstName(String firstName, String middleInitial) throws MissingNameException {
        ...
        int sum = 0;
        if (firstName.isBlank()) throw new MissingNameException("The first name cannot be blank");
        else if (firstNameCodeMap.containsKey(firstName)) {
            int first = firstNameCodeMap.get(firstName);
            int middle = middleInitialCodeMap.get(middleInitial);
            sum = first + middle;
        }
        else if (!firstNameCodeMap.containsKey(firstName))
        {
            int first = firstInitialCodeMap.get(firstName.substring(0,1));
            int middle = middleInitialCodeMap.get(middleInitial);
            sum = first + middle;
        }
        else if (middleInitial.isBlank() && (firstNameCodeMap.containsKey(firstName))){
            sum = firstNameCodeMap.get(firstName);
        }
        else if (middleInitial.isBlank() && (!firstNameCodeMap.containsKey(firstName))){
            sum = firstInitialCodeMap.get(firstName.substring(0,1));
        }
